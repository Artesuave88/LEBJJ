...
export async function sendEmailNotification(
  ...
  if (!to) {
    return {
      ok: false,
      message:
        "Missing recipient email. Set TRIAL_TO_EMAIL (or CONTACT_TO_EMAIL) in environment variables.",
    };
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return {
        ok: false,
        message:
          "Unable to send booking notification right now. Please try again shortly.",
      };
    }
